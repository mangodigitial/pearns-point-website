...
// Icon keys map to the SVGs used in the contact cards. The Sanity `icon`
// field stores one of these keys (falls back to the location pin).
const contactIcons: Record<string, React.ReactNode> = {
  phone: <svg viewBox="0 0 24 24" className="w-[22px] h-[22px] stroke-ocean stroke-[1.3] fill-none"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg>,
  email: <svg viewBox="0 0 24 24" className="w-[22px] h-[22px] stroke-ocean stroke-[1.3] fill-none"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg>,
  location: <svg viewBox="0 0 24 24" className="w-[22px] h-[22px] stroke-ocean stroke-[1.3] fill-none"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" /></svg>,
}

const defaultContactCards = [
  { icon: 'phone', title: 'Call Us', subtitle: 'Speak directly with our sales team', linkText: '+1 268-736-4028', linkHref: 'tel:+12687364028', detail: 'Antigua Office' },
  { icon: 'email', title: 'Email Us', subtitle: 'We respond within 24 hours', linkText: 'info@orangelimited.com', linkHref: 'mailto:info@orangelimited.com', detail: 'General & Sales Enquiries' },
  { icon: 'location', title: 'Visit Us', subtitle: 'Tour the peninsula in person', linkText: 'Book a Site Visit', linkHref: '/contact', detail: 'Pearns Point, Five Islands, Antigua' },
]
...
export default function ContactPage({ cmsData }: Props) {
  ...
  // Contact cards
  const contactCards = cmsData?.contactCards?.length ? cmsData.contactCards : defaultContactCards
      ...
      <section className="pt-20 max-lg:pt-[60px] px-[60px] max-lg:px-7 max-w-content mx-auto">
        <StaggerReveal className="grid grid-cols-3 max-lg:grid-cols-1 gap-6 max-lg:gap-4">
          {contactCards.map((card: any, i: number) => {
            const actionClass = 'text-[0.82rem] font-medium text-ocean hover:text-ocean-deep transition-colors duration-300'
            const isInternal = card.linkHref?.startsWith('/')
            return (
              <ContactCard
                key={i}
                icon={contactIcons[card.icon] || contactIcons.location}
                title={card.title}
                description={card.subtitle}
                action={
                  isInternal ? (
                    <Link href={card.linkHref} className={actionClass}>{card.linkText}</Link>
                  ) : (
                    <a href={card.linkHref} className={actionClass}>{card.linkText}</a>
                  )
                }
